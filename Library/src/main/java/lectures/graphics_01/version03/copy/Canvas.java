...
public class Canvas extends JPanel
{
    ...
    double      rWidth  = 3.0 * currWidth / 4.0;
    double      rHeight = 3.0 * currHeight / 4.0;
    double      rXco    = (currWidth - rWidth) / 2.0;
    double      rYco    = (currHeight - rHeight) / 2.0;
    Rectangle2D rect    = 
        new Rectangle2D.Double( rXco, rYco, rWidth, rHeight );
    gtx.setColor( rFill );
    gtx.fill( rect );
    gtx.setColor( rEdge );
    gtx.draw( rect );
