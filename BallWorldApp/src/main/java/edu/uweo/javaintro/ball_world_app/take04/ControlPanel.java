...
//        add( new JLabel( "Pause" ) );
    ...
    /**
     * Processes a given ChangeEvent generated by
     * a change in the value of the slider
     * that controls the speed of the ball.
     * 
     * @param evt
     */
    private void adjustSlider( ChangeEvent evt )
    {
        // Make sure this is really a JSlider
        // (don't know why it would be anything else,
        // but safety first).
        Object  source  = evt.getSource();
        if ( !(source instanceof JSlider) )
        {
            JSlider slider  = (JSlider)source;
            // Don't propagate changes while the slider is in motion
            if ( !slider.getValueIsAdjusting() )
            {
                double  sliderPosition  = slider.getValue();
                double  sliderPercent   = sliderPosition / slider.getMaximum();
                ballField.getBall().setSpeed( sliderPercent );
            }
        }
    }
}
